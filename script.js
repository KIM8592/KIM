...
const filterComments = (text) => {
  ...
  // BƯỚC 1: Thay thế các cụm "cô/thầy + động từ + em"
  const replacements = [
  ...
  // BƯỚC 2: Xóa từ "cô/thầy" đơn lẻ còn sót
  filtered = filtered.replace(/\bcô\b/g, '');
  filtered = filtered.replace(/\bthầy\b/g, '');
  filtered = filtered.replace(/\bgiáo\s+viên\b/gi, '');
  filtered = filtered.replace(/\bcô\s+giáo\b/gi, '');
  filtered = filtered.replace(/\bthầy\s+giáo\b/gi, '');

  // BƯỚC 3: Xóa space thừa
  filtered = filtered.replace(/\s+/g, ' ').trim();
  
  // BƯỚC 4: Xóa dấu phẩy/chấm thừa ở đầu
  ...
  for (let i = 0; i < allTargets.length; i += BATCH_SIZE) {
    ...
    console.log(`📢 Batch ${batchNum}/${totalBatches}: ${batch.length} học sinh`);
...
HƯỚNG DẪN VIẾT NHẬN XÉT HỌC SINH TIỂU HỌC:

🎯 NGUYÊN TẮC CƠ BẢN:
- Viết cho học sinh, phụ huynh dễ hiểu, không dùng thuật ngữ phức tạp
- Bắt đầu bằng "Em"
- Không nhắc tên học sinh, không dùng "cô/thầy", không nêu lại tên môn học/tiêu chí
- KHÔNG DÙNG "cô", "thầy", "giáo viên" trong câu nhận xét
- Mỗi nhận xét phải KHÁC NHAU hoàn toàn, không lặp cấu trúc câu
- Viết tự nhiên như giáo viên nói chuyện trực tiếp với học sinh

🌟 KHI VIẾT NHẬN XÉT - NÓI VỀ NHỮNG GÌ HỌC SINH ĐÃ LÀM ĐƯỢC:
- THAY VÌ: "Em học Toán tốt"
- HÃY: "Em giải được các bài toán về phép cộng, trừ trong phạm vi 20, biết tìm số còn thiếu"

- THAY VÌ: "Em học Tiếng Việt khá"
- HÃY: "Em đọc thuộc lòng nhiều bài thơ, viết được câu hoàn chỉnh với chủ ngữ, vị ngữ, tân ngữ"

- THAY VÌ: "Em chăm chỉ"
- HÃY: "Em tham gia tích cực vào các hoạt động tập thể, luôn hoàn thành bài tập về nhà"
...
❌ VIẾT SAI (chung chung, không cụ thể):
"Em học Toán tốt, chăm chỉ học tập, cần cố gắng hơn"

✅ VIẾT ĐÚNG (cụ thể, nêu kiến thức/kỹ năng):
"Em tính được phép cộng, trừ trong phạm vi 100 chính xác, hiểu cách đặt tính hàng dọc. Tiếp tục luyện bài toán có lời văn để em giải tốt hơn"

❌ VIẾT SAI:
"Em học Tiếng Việt khá"

✅ VIẾT ĐÚNG:
"Em đọc được các câu chứa chữ mới, nắm được đặc điểm của các loài động vật qua bài. Nên tập viết thêm câu để luyện kỹ năng dùng từ ngữ"

🔄 CHỐNG TRÙNG LẶP - BẮT BUỘC:
- Nếu 3 em học Tiếng Việt mức T, phải viết 3 cách hoàn toàn khác nhau
- Không dùng lại cấu trúc: "Em đọc tốt, viết tốt, chăm chỉ" cho nhiều em
- Thay đổi:
  + Cách mở đầu: Em đã nắm / Em biết / Em thực hiện / Em giải được
  + Từ khen: không lặp "chăm chỉ", thay đổi giữa "tích cực, chủ động, chịu khó"
  + Cách nêu hướng: "Tiếp tục luyện / Nên tập thêm / Hãy rèn / Cần đặc biệt chú ý"
...
`;
// ✅ THÊM PHẦN NÀY (ngay sau systemPrompt)
const subjectGuidance = {
  'Tiếng Việt': `
  KIẾN THỨC/KỸ NĂNG CẦN NHẬN XÉT:
  - Đọc: đọc lưu loát, hiểu ý chính, đọc to, giọng đúng
  - Viết: viết được câu hoàn chỉnh, viết chữ đúng hình thể, viết sạch sẽ
  - Nói: nói rõ ràng, tự tin, diễn đạt tròn trịa
  - Chính tả: viết chính xác, không sai chính tả
  - VÍ DỤ: "Em đọc bài thơ lưu lotas, phát âm đúng, hiểu ý tình của tác phẩm"
  `,
  'Toán': `
  KIẾN THỨC/KỸ NĂNG CẦN NHẬN XÉT:
  - Tính toán: cộng, trừ, nhân, chia chính xác
  - Giải toán: hiểu đề, tìm cách giải, trình bày hợp lý
  - Hình học: nhận biết hình, tính chu vi, diện tích
  - VÍ DỤ: "Em giải được bài toán có hai phép tính, trình bày rõ ràng từng bước"
  `,
  'Khoa học': `
  KIẾN THỨC/KỸ NĂNG CẦN NHẬN XÉT:
  - Quan sát: khám phá, tò mò, hỏi các câu hóc búa
  - Thí nghiệm: thực hiện theo hướng dẫn, ghi chép kết quả
  - Kết luận: rút ra nhận xét từ quan sát
  - VÍ DỤ: "Em tham gia tích cực vào hoạt động thí nghiệm, ghi chép kỹ lưỡng kết quả"
  `,
  'Thể dục': `
  KIẾN THỨC/KỸ NĂNG CẦN NHẬN XÉT:
  - Kỹ năng: chạy, nhảy, bắt bóng, theo quy tắc
  - Sức khỏe: tham gia đầy đủ, không sợ khó
  - VÍ DỤ: "Em chạy nhanh, tham gia tích cực vào trò chơi, biết bảo vệ bản thân"
  `,
  'Mĩ thuật': `
  KIẾN THỨC/KỸ NĂNG CẦN NHẬN XÉT:
  - Vẽ: vẽ đúng tỉ lệ, màu sắc hài hòa
  - Sáng tạo: tự tạo các ý tưởng, độc đáo
  - VÍ DỤ: "Em vẽ được các hình vật với tỉ lệ hợp lý, tô màu đẹp mắt"
  `
};

    // ✅ Lấy hướng dẫn môn học
const subName = subjects.find(s=>s.id===selectedSubId)?.name || "";
const guidance = subjectGuidance[subName] || "";

// ✅ Phân tích ghi chú của giáo viên
const teacherNotes = aiPrompt || "";
const analysisPrompt = teacherNotes ? `
PHÂN TÍCH GHI CHÚ CỦA GIÁO VIÊN:
"${teacherNotes}"

Dựa vào ghi chú này, hãy:
1. Xác định các yêu cầu riêng của giáo viên (nên khen/góp ý điều gì)
2. Xác định phong cách viết nhận xét mong muốn
3. Áp dụng vào từng học sinh theo mức đánh giá của họ

ƯUTIÊN:
- Nếu ghi chú nói "khen sự cố gắng", hãy nhấn mạnh vào nỗ lực, không chỉ kết quả
- Nếu ghi chú nói "cần phát triển kỹ năng X", hãy nêu hướng cải thiện kỹ năng đó
- Nếu ghi chú nói "khuyến khích sáng tạo", hãy khen những ý tưởng độc đáo của em
` : ``;

const userInstruction = `${analysisPrompt}
...
Hãy viết nhận xét cho từng học sinh theo đúng ID và trả về đúng ${studentContexts.length} nhận xét.`;
    ...
    try {
      ...
      // ✅ BƯỚC 2: Thêm error handling chi tiết
      if (result.error) {
        const errorMessage = result.error.message || "";
        console.error(`❌ Batch ${batchNum} lỗi:`, errorMessage);

        // Kiểm tra lỗi cụ thể
        if (errorMessage.includes("401") || errorMessage.includes("Invalid API Key")) {
          showToast('❌ API Key không hợp lệ!', "error", "🔑", 5000);
          setShowApiKeyModal(true);
          setIsGenerating(false);
          return;
        } else if (errorMessage.includes("429") || errorMessage.includes("quota")) {
          showToast('❌ Hết quota API! Thử lại sau.', "error", "📊", 5000);
          setIsGenerating(false);
          return;
        } else {
          showToast(`⚠️ Batch ${batchNum} lỗi. Tiếp tục batch tiếp...`, "warning", "⚠️", 3000);
          failedStudents = [...failedStudents, ...batch]; // ✅ Lưu học sinh bị lỗi
          await delay(2000); // Dừng 2 giây trước khi tiếp
          continue; // ✅ TIẾP TỤC: continue thay vì break
        }
      }
      ...
      if (jsonText) {
        ...
        try {
          ...
          console.log(`✅ Batch ${batchNum} thành công: ${Object.keys(results).length} học sinh`);
          
        } catch (parseError) {
          console.error(`❌ Parse JSON batch ${batchNum}:`, parseError);
          showToast(`Batch ${batchNum}: Lỗi parse JSON`, "error", "❌", 3000);
          failedStudents = [...failedStudents, ...batch]; // ✅ Lưu học sinh bị lỗi
          await delay(2000);
          continue; // ✅ TIẾP TỤC
        }
      ...
      showToast(`Batch ${batchNum}: Lỗi kết nối`, 'error', '❌', 3000);
      failedStudents = [...failedStudents, ...batch]; // ✅ Lưu học sinh bị lỗi
      await delay(2000);
      continue; // ✅ TIẾP TỤC
    }

    // ✅ BƯỚC 4: Dừng 1 giây giữa các batch
    if (i + BATCH_SIZE < allTargets.length) {
      await delay(1000);
    }
  }

  // ✅ BƯỚC 5: Retry các học sinh bị bỏ sót
  if (failedStudents.length > 0) {
    console.log(`⚠️ ${failedStudents.length} học sinh bị lỗi, thử lại...`);
    showToast(`Thử lại ${failedStudents.length} học sinh bị lỗi...`, 'info', '🔄', 2000);
    
    await delay(3000); // Chờ 3 giây trước retry
    
    // Retry từng học sinh 1 lần
    for (const stuRetry of failedStudents) {
      try {
        const studentContexts = [stuRetry].map(stu => {
          const d = studentData[stu.id] || {};
          const draft = draftData[stu.id] || {};
          let info = "";
...
const userInstruction = `Ghi chú riêng của giáo viên: ${aiPrompt}

${guidance}

Viết nhận xét cho 1 học sinh này:
${JSON.stringify(studentContexts)}

HƯỚNG DẪN:
1. Viết nhận xét nêu rõ KIẾN THỨC/KỸ NĂNG HỌC SINH ĐÃ LÀM ĐƯỢC
2. Không nêu tên môn học
3. Viết tự nhiên, cụ thể, dễ hiểu`;

        const resRetry = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite-preview:generateContent?key=${apiKey}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ 
            contents: [{ parts: [{ text: userInstruction }] }], 
        ...
        if (!resultRetry.error) {
          const jsonRetry = resultRetry.candidates?.[0]?.content?.parts?.[0]?.text;
          if (jsonRetry) {
            let cleanRetry = jsonRetry.trim();
            if (cleanRetry.startsWith("```")) {
              cleanRetry = cleanRetry.replace(/```json|```/g, "").trim();
            }
            const resultsRetry = JSON.parse(cleanRetry);
            
            setDraftData(prev => ({
              ...prev,
              [stuRetry.id]: {
                ...(prev[stuRetry.id] || {}),
                comment: resultsRetry[stuRetry.id]
              }
            }));
            
            successCount++;
            console.log(`✅ Retry thành công: ${stuRetry.name}`);
          }
        }
      } catch (retryErr) {
        console.error(`❌ Retry thất bại ${stuRetry.name}:`, retryErr);
      }
  ...
  // ✅ BƯỚC 6: Thông báo kết quả cuối cùng
  if (failedStudents.length === 0) {
    showToast(`✅ Tạo nhận xét thành công cho ${successCount}/${allTargets.length} học sinh`, 'success', '✅', 3000);
  } else {
    const stillFailed = allTargets.length - successCount;
    showToast(`✅ ${successCount}/${allTargets.length} thành công | ⚠️ ${stillFailed} vẫn lỗi`, 'warning', '⚠️', 5000);
  }
};
...
🚪 Đăng xuất
  ...
  <input className="w-full bg-slate-50 border-2 border-slate-300 rounded-xl pl-12 pr-4 py-4 text-sm font-semibold outline-none focus:border-indigo-500 transition-all" 
    placeholder="VD: Khoảng 20 từ | Ngắn gọn | Chi tiết | Khen sự cố gắng, khoảng 30 từ" 
    value={aiPrompt} 
